...
for t=1:trials
    ...
    [G_grad,sig_grad,pr_grad] = LearningParams.compute_gradients_elbo_modified(params,mu_vb,stim,z_hidden);
    ...
    for i=1:params.Neurons_hidden
        for p=1:params.pix^2
            ...
            log_p = LearningParams.compute_log_p(params,stim,z_hidden);
            log_q = LearningParams.compute_log_q(params,z_hidden,mu_vb);
            tot_log_prob_G_plus = log_prob - (1 - log_p +log_q) * sum(z_hidden(:) .* log(mu_vb_G_plus(:)) + (1 - z_hidden(:)) .* log(1-mu_vb_G_plus(:)));
            
            params_G = params;
            params_G.G(p,i) = params.G(p,i) - epsilon;
            mu_vb_G_minus = LearningParams.variational_bayes(params_G,stim);
            log_prob = LearningParams.compute_log_p(params_G,stim,z_hidden);
            tot_log_prob_G_minus = log_prob - (1 - log_p +log_q) * sum(z_hidden(:) .* log(mu_vb_G_minus(:)) + (1 - z_hidden(:)) .* log(1-mu_vb_G_minus(:)));
            
            G_slope(t,p,i) = (tot_log_prob_G_plus - tot_log_prob_G_minus)/(2*epsilon);
        end
    end
    ...
    log_p = LearningParams.compute_log_p(params,stim,z_hidden);
    log_q = LearningParams.compute_log_q(params,z_hidden,mu_vb);
    tot_log_prob_sig_plus = log_prob - (1 - log_p +log_q) * sum(z_hidden .* log(mu_vb_sig_plus) + (1-z_hidden) .* log(1-mu_vb_sig_plus));
    params_sig = params;
    params_sig.sigma_stim = params.sigma_stim - epsilon;
    log_prob = LearningParams.compute_log_p(params_sig,stim,z_hidden);
    mu_vb_sig_minus = LearningParams.variational_bayes(params_sig,stim);
    tot_log_prob_sig_minus = log_prob - (1 - log_p +log_q) * sum(z_hidden .* log(mu_vb_sig_minus) + (1-z_hidden) .* log(1-mu_vb_sig_minus));
    sig_slope(t) = (tot_log_prob_sig_plus - tot_log_prob_sig_minus)/(2*epsilon);
    ...
    log_p = LearningParams.compute_log_p(params,stim,z_hidden);
    log_q = LearningParams.compute_log_q(params,z_hidden,mu_vb);
    tot_log_prob_prior_plus = log_prob - (1 - log_p +log_q) * sum(z_hidden(:) .* log(mu_vb_prior_plus(:)) + (1 - z_hidden(:)) .* log(1-mu_vb_prior_plus(:)));
    params_prior = params;
    params_prior.prior = params.prior - epsilon;
    log_prob = LearningParams.compute_log_p(params_prior,stim,z_hidden);
    mu_vb_prior_minus = LearningParams.variational_bayes(params_prior,stim);
    tot_log_prob_prior_minus = log_prob - (1 - log_p +log_q) * sum(z_hidden(:) .* log(mu_vb_prior_minus(:)) + (1 - z_hidden(:)) .* log(1-mu_vb_prior_minus(:)));
    pr_slope(t) = (tot_log_prob_prior_plus - tot_log_prob_prior_minus)/(2*epsilon);
